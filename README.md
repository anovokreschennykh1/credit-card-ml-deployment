 ...
 **Архитектура сервиса:** В данном проекте используется **монолитный подход**. То есть все этапы от обучения до хостинга модели с помощью API разворачиваются как единый сервис. Поскольку проект небольшой по размеру, то для облегчения запуска, тестирования и контейнеризации выбрана именно монолитная архитектура, нет никакой необходимости дробить отдельные этапы на микросервисы. В реальном мире для обеспечения отказоустойчивости и для дальнейшего масштабирования, обеспечения отказоустойчивости микросервисный подход мог бы оказаться более подходящим вариантом (особенно для большого банка)
 
**Концепт брокеров сообщений:** Брокер сообщений в данном проекте не используется. API обрабатывает запросы клиента в синхронном режиме от получения запроса до вывода результатов инференса. Но в продакшене можно было бы добавить брокер сообщений, например RabbitMQ или Kafka для:
